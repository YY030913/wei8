...
function cloud_m_prepare($name) {
	...
	$pars['module'] = $name;
	$dat = cloud_request('http://www.baidu.com/gateway.php', $pars);
	if (is_error($dat)) {
		return $dat;
	}
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://www.baidu.com/gateway.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://www.baidu.com/gateway.php', $pars, $headers, 300);
	if(is_error($dat)) {
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://www.baidu.com/gateway.php', $pars);
...
function cloud_auth_url($forward, $data = array()){
	...
	$auth_url = 'http://www.baidu.com/web/index.php?c=auth&a=passwort&__auth=' . $query;
